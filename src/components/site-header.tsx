    ...
    <header className="sticky top-0 z-50 bg-background/85 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          ...
          >
            <button className="flex items-center gap-1 rounded-sm text-sm font-medium text-foreground/80 transition-colors duration-150 hover:text-primary">
              Products
              <ChevronDown
                className={`h-3.5 w-3.5 transition-transform duration-200 ${prodOpen ? "rotate-180" : ""}`}
              />
            </button>
            {prodOpen && (
              <div className="absolute left-1/2 top-full w-56 -translate-x-1/2 pt-3">
                <div className="animate-panel-in overflow-hidden rounded-xl border border-border bg-popover shadow-xl shadow-black/40">
                  <Link
                    to="/products/assure"
                    className="block px-4 py-3 transition-colors duration-150 hover:bg-muted"
                  >
                  ...
                  <a
                    ...
                    className="block border-t border-border px-4 py-3 transition-colors duration-150 hover:bg-muted"
                  >
        ...
        <div className="hidden md:block">
          <Link
            ...
            className="inline-flex items-center rounded-md gradient-brand px-4 py-2 text-sm font-medium text-primary-foreground shadow-md shadow-primary/20 transition-[box-shadow,transform] duration-200 hover:shadow-lg hover:shadow-primary/30 active:translate-y-px"
          >
            Request a Demo
          </Link>
        </div>
      ...
      {open && (
        <div className="animate-panel-in border-t border-border bg-background md:hidden">
          <div className="mx-auto flex max-w-7xl flex-col gap-1 px-6 py-4">
            ...
            <Link
              ...
              hashScrollIntoView={{ behavior: "smooth" }}
              onClick={() => setOpen(false)}
              className="mt-2 inline-flex items-center justify-center rounded-md gradient-brand px-4 py-2 text-sm font-medium text-primary-foreground shadow-md shadow-primary/20 transition-[box-shadow,transform] duration-200 hover:shadow-lg hover:shadow-primary/30 active:translate-y-px"
            >
              Request a Demo
            </Link>
          </div>
...
function NavLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link
      to={to}
      className="rounded-sm text-sm font-medium text-foreground/80 transition-colors duration-150 hover:text-primary"
